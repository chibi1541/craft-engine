...
void Engine::Run()
{
	...
	while (true)
	{
		...
		if (deltaTime >= oneFrameTime)
		{
			...
			// 다음 프레임의 기준점은 "이번 프레임이 끝난 시각"이 아니라
			// "이번 프레임이 시작한 시각"이다.
			//
			// 여기서 시간을 다시 읽으면 deltaTime이
			// "직전 프레임이 끝난 뒤 기다린 시간"만 되고
			// 프레임 자신의 작업 시간이 통째로 빠진다. 그러면 세 가지가 깨진다.
			//
			//  1. UpdateFps가 항상 목표 프레임레이트를 찍어서
			//     성능이 떨어져도 그걸 영영 보여주지 못한다.
			//  2. Tick에 넘기는 deltaTime이 실제 경과 시간보다 작아져,
			//     프레임이 무거워지면 게임 시간이 실시간보다 느려진다.
			//  3. 작업 시간 "위에" 한 프레임치 대기가 더 붙어
			//     이미 느린 프레임을 더 느리게 만든다.
			//
			// 시작 시각을 기준으로 잡으면 deltaTime은 프레임 한 바퀴가
			// 실제로 걸린 전체 시간(작업 + 대기)이 된다.
			previous = current;
		}
